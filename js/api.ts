import type { FetchReposOptions, FetchReposResult, CacheData, GitHubRepo } from './types.js';
...
function loadCache(user: string): CacheData | null {
  try {
...
function saveCache(user: string, { etag, data }: { etag: string; data: GitHubRepo[] }): void {
  try {
    localStorage.setItem(
      cacheKey(user),
      JSON.stringify({ ts: Date.now(), etag: etag || '', data })
    );
  } catch {
...
export async function fetchAllRepos({ user, perPage, signal }: FetchReposOptions): Promise<FetchReposResult> {
  try {
